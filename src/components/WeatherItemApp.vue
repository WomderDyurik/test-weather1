<template>
  <div class="item">
    <div class="item__city">{{ `${city.name}, ${city.sys.country}` }}</div>
    <div class="item__features">
      <img
        class="item__features-image"
        :src="`https://openweathermap.org/img/wn/${city.weather[0].icon}@2x.png`"
      />
      <div class="item__features-deg">{{ computedDeg(city.main.temp) }}</div>
    </div>
    <div class="item__feel">
      <div class="item__feels">
        {{ `Feels like ${computedDeg(city.main.feels_like)}.` }}
      </div>
      <div class="item__feels">{{ allDescriptions(city.weather) }}</div>
    </div>

    <div class="item__descriptions">
      <div class="item__description wind">
        {{ `${city.wind.speed}m/s` }}
      </div>
      <div class="item__description pressure">{{ `${city.main.pressure}hPa` }}</div>
...
import { Weather, WeatherElement } from "@/types/WeatherTypes.interface";
defineComponent({
  name: "ItemApp",
});
...
function computedDeg(temp: number): string {
  return `${Math.round(temp - 273)}°C`;
}

function allDescriptions(arr: WeatherElement[]): string {
